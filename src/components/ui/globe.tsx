...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  const defaultProps = {
    pointSize: 1,
    atmosphereColor: "#a7f3d0", // Verde claro para atmosfera
    showAtmosphere: true,
    atmosphereAltitude: 0.1,
    polygonColor: "rgba(34, 197, 94, 0.7)", // Verde semi-transparente
    globeColor: "#064e3b", // Verde escuro para o globo
    emissive: "#065f46", // Verde muito escuro para emissão
...
export function WebGLRendererConfig() {
  ...
  useEffect(() => {
    ...
    gl.setClearColor(0xa7f3d0, 0); // Verde claro para clear color
  }, []);
...
export function World(props: WorldProps) {
  ...
  scene.fog = new Fog(0xa7f3d0, 400, 2000); // Verde claro para fog
  return (
    <Canvas scene={scene} camera={new PerspectiveCamera(50, aspect, 180, 1800)}>
      <WebGLRendererConfig />
      <ambientLight color={globeConfig.ambientLight || "#86efac"} intensity={0.6} />
      <directionalLight
        color={globeConfig.directionalLeftLight || "#4ade80"}
        position={new Vector3(-400, 100, 400)}
      />
      <directionalLight
        color={globeConfig.directionalTopLight || "#22c55e"}
        position={new Vector3(-200, 500, 200)}
      />
      <pointLight
        color={globeConfig.pointLight || "#16a34a"}
        position={new Vector3(-200, 500, 200)}
        intensity={0.8}
      />
      <Globe {...props} />
